...
- コピーしたテキスト / 画像 / ファイルを 0.3 秒以内に履歴へ追加（macOS 標準の ⌃⇧⌘4 も画像として入る）
- メニューまたはパネルのボタンから範囲キャプチャ。⌘⇧2 は初期 OFF（メニューで ON にできる）
...
範囲キャプチャを初めて使うときに「画面収録」の許可が求められるので許可し、もう一度キャプチャしてください。
...
| メニューバー | パネル表示切替、⌘⇧2 ショートカット、監視の一時停止、全消去、ログイン時起動 |
